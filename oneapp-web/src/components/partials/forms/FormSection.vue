<template>
    <validation-observer class="validated-form" ref="observer" v-slot="{ handleSubmit }">
        ...
        <us-form @submit="handleSubmit(doSubmit)" v-if="formData" size="lg" class="mt-5 pb-3">
            <!--
            <span v-for="(section, sectionIndex) in currentPage.sections" :key="sectionIndex">            
            ...
            <!-- SUPPORT FOR A SLOT -->
            <span v-if="currentPage.slotId">
                <slot :name="currentPage.slotId" v-bind:formData="currentPage" />
            </span>

            <span v-else>
                <span v-for="(item, index) in currentPage.fields" :key="index">
